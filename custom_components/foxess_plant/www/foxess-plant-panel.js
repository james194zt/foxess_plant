...
const WORK_MODE_META = {
  ...
  "Remote Control": {
    ...
    hint: "Inverter is under external remote control (read-only status on EVO).",
  },
};

const NON_SELECTABLE_WORK_MODES = new Set([
  "Force Charge",
  "Force Discharge",
  "Remote Control",
  "unknown",
...
function selectableWorkModeOptions(options) {
  return (options || []).filter((opt) => !NON_SELECTABLE_WORK_MODES.has(String(opt)));
}

function resolveWorkModeDraft(current, options) {
  const selectable = selectableWorkModeOptions(options);
  if (current && !NON_SELECTABLE_WORK_MODES.has(String(current))) return current;
  return selectable[0] ?? current ?? "Self Use";
}
...
function foxWorkModeLabel(hass, plant, plantState) {
  const map = { ...(plant?.entity_map || {}), ...(plantState?.entity_map || {}) };
  const rc = plantState?.settings?.remote_control;
  if (rc && rc !== "Disable" && rc !== "unknown" && rc !== "unavailable") {
    return rc;
  }
...
<div class="panel-brand-sub">FoxESS inverter control</div>
  ...
  _initWorkModeDraft() {
    ...
    this._workModeDraft = resolveWorkModeDraft(current, options);
  }
