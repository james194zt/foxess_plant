"""Grid import charging — cheapest window and negative-import arbitrage."""
...
from .solcast_remaining import solcast_remaining_kwh
...
def evaluate_grid_charge(
    *,
    ...
    effective_target = target_soc_pct
    if effective_target is None:
        effective_target = float(getattr(config, "target_soc", 100.0) or 100.0)
    max_target = float(getattr(config, "max_target_soc", None) or getattr(config, "target_soc", 100.0) or 100.0)
    effective_target = min(effective_target, max_target)
    ...
    if target_max_soc is None:
        target_max_soc = min(effective_target, max_target)

    base_kwargs = {
        "operating_mode": operating_mode,
        "reserve_kwh": reserve_kwh,
        "exportable_kwh": exportable_kwh,
        "target_soc_effective": round(effective_target, 1),
        "grid_gap_kwh": grid_gap_kwh,
        "dark_hours_kwh": dark_hours_kwh,
        "eval_tier": eval_tier,
    ...
    if deficit <= min_deficit:
        return SmartChargeDecision(
            action="skip",
            reason=f"Battery near target ({deficit:.1f} kWh deficit, goal {effective_target:.0f}% SOC)",
