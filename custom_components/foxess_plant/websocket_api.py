...
def async_register_ws_handlers(hass: HomeAssistant) -> None:
    ...
    if DEBUG_MODBUS_PROBE:
        ...
        @websocket_api.websocket_command(
            {
                vol.Required("type"): WS_TYPE_RUN_MODBUS_DEBUG_PROBE,
                vol.Optional("plant_id"): str,
            }
        )
        @websocket_api.require_admin
        @websocket_api.async_response
        async def ws_run_modbus_debug_probe(
            hass: HomeAssistant,
            connection: websocket_api.ActiveConnection,
            msg: dict[str, Any],
        ) -> None:
            ...
            if coordinator is None:
                ...
                return
            try:
                result = await coordinator.async_run_modbus_debug_probe()
            except HomeAssistantError as err:
                connection.send_error(msg["id"], "modbus_debug_probe_failed", str(err))
                return
            except Exception as err:
                _LOGGER.exception("Modbus debug probe failed")
                connection.send_error(msg["id"], "modbus_debug_probe_failed", str(err))
                return
            connection.send_result(
                msg["id"],
