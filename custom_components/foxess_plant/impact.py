...
# Industry estimates used by FoxESS Cloud for marketing impact figures.
CO2_KG_PER_KWH = 1.0
OIL_LITRES_PER_KWH = 0.22
# Fox app shows "trees planted" ≈ CO₂ offset in kg (conceptual, not literal saplings).
...
def compute_impact(entity_states: dict[str, str | None]) -> dict[str, Any]:
    """Estimate CO₂, oil, and trees from lifetime solar kWh (Fox Cloud formulas)."""
    kwh = lifetime_solar_kwh(entity_states)
    if kwh <= 0:
        return {}
