...
4. Overview and Energy charts use the native PV forecast only — you can uninstall any third-party Solcast HA integration.
5. Diagnostic sensors mirror the ha-solcast-solar set (forecast today/tomorrow, remaining today, power now, peak times, etc.) with `detailedForecast` attributes for Lovelace cards — no separate Solcast integration required.
